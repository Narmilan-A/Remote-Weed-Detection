...
for i, model in enumerate(models):
   ...
   # Plot the confusion matrix for the current model using heatmap()
    ...
    # Develop classification report only using class id=1,2
    cr = classification_report(y_test, y_pred, target_names=["Hw Flowers", "Background"])
...
# Save the figure
plt.tight_layout()
plt.savefig('Training model outcomes/training_CM_heatmap_subplot.png', bbox_inches='tight', dpi=300)
plt.show()
...
for i, roi in enumerate(rois):
    input_img_file = 'C:/Users/N10837647/OneDrive - Queensland University of Technology/Desktop/nz_hw_rs/flower_classification/rois/nz_hw_roi_{}.tif'.format(roi)
    input_mask_file = 'C:/Users/N10837647/OneDrive - Queensland University of Technology/Desktop/nz_hw_rs/flower_classification/masks/nz_hw_mask_{}.tif'.format(roi)
    ds_img = gdal.Open(input_img_file)
    ds_mask = gdal.Open(input_mask_file)
    input_img = np.array([ds_img.GetRasterBand(j+1).ReadAsArray() for j in range(5)])
    input_img= np.transpose(input_img, (1,2,0))
...
colors_legend= ['#000000', '#ff0000']
cmap_legend = ListedColormap(colors_legend)

#Create legend 
labels_legend = ["Hw Flowers", "Background"]
